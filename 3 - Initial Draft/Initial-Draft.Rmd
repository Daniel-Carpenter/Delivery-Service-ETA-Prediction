...
df.train.neural <- df.train.base %>%
  dplyr::select(c('Delivery_Person_Age', 'Delivery_Person_Ratings', 'Target_Variable'))
dim(df.train.neural)
```

```{r, echo=FALSE, results='hide'}

df.train.neural <- na.omit(df.train.neural)

minb = apply(df.train.neural, 2, min)
maxb = apply(df.train.neural, 2, max)

train.neural.scaled = as.data.frame(scale(df.train.neural, center = minb, scale = maxb-minb))

summary(train.neural.scaled)

fmla1 = formula(paste("Target_Variable~", paste(names(train.neural.scaled[,c(-14)]), collapse= '+')))
